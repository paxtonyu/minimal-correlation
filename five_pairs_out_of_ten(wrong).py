...
def load_data(file_path):
    ...
    original_data = data.iloc[:10, 1:]  # 0~9row, 1~end columns
...
def compute_lowest_correlations(data, N):
    ...
    for i in range(rows):
        for j in range(i + 1, rows):
            row_i = data.iloc[i, :].values  # 0~9 rows,
            row_j = data.iloc[j, :].values  # all columns
...
if __name__ == "__main__":
    ...
    for (i, j), corr in lowest_correlations:
        print(f"Rows {i+1} and {j+1} with correlation {corr:.6f}")
